defmodule LiaisonServerWeb.DataSpaceChannel do
  ...
  @impl true
  def handle_in("action", %{"type" => "UpdateSourceSchema"} = action, socket), do: handle_action(&MetaStore.update_source_schema/2, action, socket)
  ...
  @impl true
  def handle_in("action", %{"type" => "DeleteConcept"} = action, socket), do: handle_action(&MetaStore.delete_concept/2, action, socket)
